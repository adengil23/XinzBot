...
module.exports = async(xinz, anj, welcome, left) => {
    ...
    if (anj.action === 'add'){
        if (anj.participants[0] === xinz.user.jid){
            ...
            xinz.sendMessage(anj.jid, `Hai Saya ${Gilang Store}, silahkan kirim #menu`, MessageType.text)
        } else if (isWelcome){
            try {
            ...
            xinz.sendMessage(anj.jid, await getBuffer(pic), MessageType.image, {caption: `Hai @${anj.participants[0].split("@")[0]}, selamat datang di ${CLIENT SUNDACITYHOST}`, contextInfo: {"mentionedJid": [anj.participants[0]]}})
        }
